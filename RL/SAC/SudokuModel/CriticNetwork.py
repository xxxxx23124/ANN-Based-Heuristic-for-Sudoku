...
class SudokuCriticInputEncoder(nn.Module):
    """负责将 (B, S, H, W, C) 与 B, S, (L * grid_size) 的输入编码为 (B, S, L, D) 的特征"""
    ...
    def forward(self, state: Tensor, action: Tensor) -> Tensor:
        ...
        state: (B, S, H, W, C)
        action: (B, S, H*W*(C-1))
        C-1是因为在x通道中有一个通道是额外信息
        C - 1 == H, C - 1 == W
        """
        B, S, H, W, C = state.shape
        action = rearrange(action, "b s (h w c) -> (b s) c h w", c=H, h=H, w=W)
        state = rearrange(state, "b s h w c -> (b s) c h w")
        x = F.gelu(self.conv(torch.cat([state, action], dim=1)))
        x = rearrange(x, "(b s) d h w -> b s (h w) d", s=S)
        # return: (B, S, L, D)
        return x

class CriticHead(nn.Module):
    """Q table 头：TimeSpace -> 输出"""
...
class TimeSpaceSudokuCriticModel(nn.Module):
    def __init__(self, config: SudokuModelConfig, device: torch.device):
        ...
        # 输入编码器
        self.encoder = SudokuCriticInputEncoder(config.input_channels, config.embed_dim, device)
    ...
    def forward(
        ...
        # 编码输入
        x_embed = self.encoder(state, action)  # state/action -> (B, S, L, D)
        ...
        # 通过价值头
        q_value, new_cache_critic = self.critic_head(
