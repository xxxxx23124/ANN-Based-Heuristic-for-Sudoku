...
class Mamba2_block(nn.Module):
    ...
    def _parallel_forward(self, u: Tensor, h: Mamba2InferenceCache) -> tuple[Tensor, Mamba2InferenceCache]:
        ...
        h.update_parallel(new_conv_input=xBC_unactivated.transpose(1, 2)[:, :, -self.args.d_conv:], # (B, D_in, 4)
    ...
    def _step(self, u: Tensor, h: Mamba2InferenceCache) -> tuple[Tensor, Mamba2InferenceCache]:
        ...
        xBC = torch.einsum("bdw,dw->bd", temp_conv_state, w)
...
def test_mamba2_block_consistency(
        batch_size: int = 2,
        seq_len: int = 64,        # 必须是偶数且 > 2，满足 _parallel_forward 里的断言
        d_model: int = 32,
        dtype=torch.float32,
    ...
    """
    检验  (1) 并行 forward  与 (2) 单步 step(+cache) 两条路径输出是否一致
    """
    torch.manual_seed(42)

    device = torch.device(device)
    cfg = Mamba2Config(d_model=d_model)
    ...
    parallel_cache=None
    y_parallel_list = []
    y_parallel_1, parallel_cache = block(x[:, :seq_len//2, :, :], parallel_cache)
    y_parallel_list.append(y_parallel_1)
    y_parallel_2, parallel_cache = block(x[:, seq_len//2:, :, :], parallel_cache)
    y_parallel_list.append(y_parallel_2)
    y_parallel = torch.cat(y_parallel_list, dim=1)
    # -------------------------------------------------------------------------
    # 2. 单步 / 循环模式
    # -------------------------------------------------------------------------
    h_step = None
    ...
    for t in range(seq_len):
        # 取第 t 个 token，维度仍保持 (B, 1, 1, D)  -> S == 1 触发 step 分支
        x_t = x[:, t:t+1, :, :]              # (B,1,1,D)
        y_t, h_step = block(x_t, h_step)     # cache 必须持续传递
        y_steps.append(y_t)                  # y_t: (B,1,1,D)

    # 沿 S 维拼接，得到 (B, S, 1, D) 与并行输出同形
    y_step = torch.cat(y_steps, dim=1)

    # -------------------------------------------------------------------------
    # 3. 比较
    # -------------------------------------------------------------------------
    if torch.allclose(y_parallel, y_step, atol=atol, rtol=rtol):
        print("✅  并行 forward 与 单步 step 输出完全一致！")
    else:
        max_abs_err = (y_parallel - y_step).abs().max().item()
        print(f"❌  两种模式输出不一致！最大绝对误差: {max_abs_err:e}")


if __name__ == "__main__":
    test_mamba2_block_consistency()